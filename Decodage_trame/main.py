# Immportation des fonctions pour le decodage des trames et l'importation des données dans la base de données 
import mysql.connector
from fonction_decodage import extraction
from decodage import decodage

fic_binaire = "Vt-DEMO_mem_observability/ethernet.result_data" # Fichier binaire ccomportant les trames à décoder 
...
print(trames) # Affichage des trames
...
for trame in trames:		
	if trame[7] == "0x800":
    	...
    	field_30, field_32, packet_date, num_test) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,
     	%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s, %s, %s,%s)""" # Requête SQL
		...
		req_arp = """INSERT INTO trame (framedate, bench_3, bench_5, framesize, mac_dest, mac_src, 
  		...
  		VALUES (%s, %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""" # Requête SQL
